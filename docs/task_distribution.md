# Task Distribution-Group 3

## Hissah Aljreawi | 444201054

### Task Role:
- Phase 1: Data & Domain Team (Problem Definition & Domain Selection,and Dataset Selection & Justification)
  
- Phase 2:
1. Generative AI Integration (Template 3 Design Documentation, Template 3 Testing Framework & Output Comparison, Template 3 Analysis) and Best Prompt Selection & Justification.
2. Integration & Documentation 

## Latifa Bahafid | 444200081

### Task Role:

- Phase 1: Supervised Learning (SVM Model: Model Selection & Rationale, Implementation & Training, 
  Hyperparameter Tuning Process & Results, Comprehensive Evaluation, and Comparative Analysis & Interpretation)
  
- Phase 2:
1. Generative AI Integration (Template 1 Design Documentation, Template 1 Testing Framework & Output Comparison, Template 1 Qualitative & Quantitative Analysis), and Generative AI Model Selection & Setup
2. Integration & Documentation 

## Lamia Alzaid | 444200913

### Task Role:
- Phase 1: Preprocessing & Visualization Team (Correlation Heatmap of Numerical Features, Class Distribution Analysis, Categorical Features vs. Loan Status, Feature Distributions, Missing Value Analysis, Feature Importance – Preliminary Analysis, and Key Feature Analysis: CIBIL Score)

- Phase 2:
1. Generative AI Integration (Template 4 Design Documentation, Template 4 Testing Framework & Output Comparison, Template 4 Qualitative & Quantitative Analysis), and Ethical Considerations & Limitations
2. Integration & Documentation

## Jana Almarshad | 444201200

### Task Role:
- Phase 1: Preprocessing & Visualization Team (Data Cleaning, Encoding Categorical and Target Variables, Outlier Detection and Analysis, Feature Engineering, Feature Scaling using StandardScaler, Save Preprocessed Dataset)

- Phase 2:
1. Unsupervised Learning (Clustering Rationale & Algorithm Selection, Data Preparation, Cluster Determination using Elbow Method & Silhouette Analysis).
2. Integration & Documentation


## Leen Alhadlaq | 444200950

### Task Role:

- Phase 1: Supervised Learning (Logistic Regression: Model Selection & Rationale, Implementation & Training, Hyperparameter Tuning Process & Results, Comprehensive Evaluation, Comparative Analysis & Interpretation) and Conclusion: Best Model Selection & Reasoning.

- Phase 2:
1. Generative AI Integration (Template 2 Design Documentation, Template 2 Testing Framework & Output Comparison, Template 2 Analysis) and Integration Plan for Final System.
2. Integration & Documentation

## Roukaya Al-Chaabouni | 444926353

### Task Role:
- Phase 1: Data & Domain Team (Exploratory Data Analysis (EDA))

- Phase 2:
1.  Unsupervised Learning (Cluster Interpretation & Profiles, Integration Strategy with the Advice System, and Challenges & Limitations).
2.  Integration & Documentation

## Layan Alrumayh | 444200378
### Task Role:
- Phase 1: Supervised Learning (Random Forest: Model Selection & Rationale, Implementation & Training, Hyperparameter Tuning Process & Results, Comprehensive Evaluation & Interpretation) and Integration Step: Random Forest Integration Planning for the Final System.
  
- Phase 2:
1.  Phase 2: Unsupervised Learning (K-Means & DBSCAN Implementation, Evaluation Metrics & Visualizations, PCA/t-SNE Cluster Visualization, and Cluster Size Distribution).
2. Integration & Documentation 
